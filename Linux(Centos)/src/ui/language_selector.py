...
class LanguageSelectorDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # [CRITICAL FIX] Ensure fonts are applied to the dialog
        ...
        try:
            ...
            print(f"Error applying fonts to selector: {e}")
    ...
    def init_ui(self):
        ...
        # 使用统一的字体管理器
        from src.utils.font_manager import apply_fonts, load_fonts
        apply_fonts(self.title_line1)
        ...
        apply_fonts(self.language_label)
        ...
        # 确保 ComboBox 及其下拉列表视图使用支持多种语言的字体
        apply_fonts(self.language_combo)
        
        # [CRITICAL FIX] 强制通过样式表设置字体和背景，解决打包后背景为纯白以及泰语显示方块的问题
        font_families_str = ""
        try:
            loaded_fonts = load_fonts()
            if loaded_fonts:
                # 构建字体族字符串，优先使用加载的字体
                font_families_str = ', '.join([f'"{f}"' for f in loaded_fonts])
        except Exception as e:
            print(f"Error loading fonts for selector: {e}")
            
        # 如果字体加载失败，使用默认字体
        if not font_families_str:
            font_families_str = '"Segoe UI", "Microsoft YaHei", "Arial", "sans-serif"'
            
        # 定义 ComboBox 样式 - 半透明白色背景，圆角，与背景融合
        # 使用 integer alpha (190/255) 确保在所有 Qt 环境中透明度正常生效
        combo_style = f"""
            ComboBox {{
                background: rgba(255, 255, 255, 190) !important;
                border: 1px solid rgba(255, 255, 255, 100);
                border-radius: 6px;
                color: #333333;
                padding: 5px 10px 5px 10px;
                text-align: left;
                font-family: {font_families_str};
                font-size: 14px;
            }}
            ComboBox:hover {{
                background: rgba(255, 255, 255, 230) !important;
            }}
            ComboBox::drop-down {{
        ...
        # 同时设置下拉列表视图的样式表
        if hasattr(self.language_combo, 'view'):
            view = self.language_combo.view()
            if view:
                apply_fonts(view)
                view.setStyleSheet(f"font-family: {font_families_str};")
        ...
        apply_fonts(self.settings_label)
        ...
        if combo_style:
             self.settings_combo.setStyleSheet(combo_style)
    ...
    def load_languages(self):
        ...
        # Nuitka 兼容：如果 BASE_DIR 已经是 src，或者需要往上一级找
        if not os.path.exists(locale_dir):
            parent_dir = os.path.dirname(BASE_DIR)
            if os.path.exists(os.path.join(parent_dir, 'src', 'locale')):
                locale_dir = os.path.join(parent_dir, 'src', 'locale')
