...
from PyQt6.Qsci import QsciScintilla, QsciLexerPython, QsciLexerBatch
from PyQt6.QtGui import QColor, QFont, QPainter, QPixmap
from PyQt6.QtCore import Qt, QProcess, QProcessEnvironment, QEvent
...
class BaseShell(QsciScintilla):
    def __init__(self, parent=None):
        ...
        self.internal_process.setProcessChannelMode(QProcess.ProcessChannelMode.MergedChannels)
    ...
    def handle_tab_completion(self, direction=1):
        ...
        for line in reversed(lines):
            ...
            if not line_clean: 
                continue
            if IS_WINDOWS:
                # Windows: PowerShell 格式 PS D:\path> 或 CMD 格式 D:\path>
                match = re.search(r'(?:PS\s+)?([a-zA-Z]:\\[^>]*)', line_clean)
            else:
                # Linux: 常见的 bash/zsh 提示格式 user@host:/path$ 或 /path$
        ...
        try:
            if os.path.exists(search_dir) and os.path.isdir(search_dir):
                ...
                for name in sorted(os.listdir(search_dir), key=str.lower):
                    # 过滤系统文件/隐藏文件
                    if IS_WINDOWS and (name.startswith('$') or name == "System Volume Information"):
                        continue
                    if IS_LINUX and name.startswith('.'):
                        continue
...
class InteractiveShell(BaseShell):
    ...
    def stop_interpreter(self):
        """停止当前的交互式 Python 解释器"""
        if self.internal_process.state() != QProcess.ProcessState.NotRunning:
    ...
    def start_process(self):
        ...
        if interpreter:
            ...
            if IS_WINDOWS:
                ...
                # Linux 使用冒号分隔 PATH
                path = env.value("PATH", "")
                new_path = f"{python_dir}:{bin_dir}:{path}"
        ...
        if IS_WINDOWS:
            ...
            # Linux 使用 bash
            self.internal_process.start("/bin/bash")
        self.internal_process.waitForStarted()
