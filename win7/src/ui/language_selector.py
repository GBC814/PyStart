...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QWidget, QGraphicsDropShadowEffect, QApplication
...
class LanguageSelectorDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.title_line1.setAlignment(Qt.AlignCenter)
        ...
        self.title_line2.setAlignment(Qt.AlignCenter)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            # PyQt5 使用 globalPos() 而不是 globalPosition()
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton and hasattr(self, '_drag_pos'):
            # PyQt5 使用 globalPos()
            new_pos = self.pos() + event.globalPos() - self._drag_pos
            self.move(new_pos)
            self._drag_pos = event.globalPos()
            event.accept()
...
def show_language_selector():
    ...
    if not app:
        ...
        # PyQt5 不支持 setHighDpiScaleFactorRoundingPolicy，属性已在 main.py 设置

    dialog = LanguageSelectorDialog()

    if dialog.exec_() == LanguageSelectorDialog.DialogCode.Accepted:
